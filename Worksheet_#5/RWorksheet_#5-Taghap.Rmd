...
```
\hfill\break
b. Using the same table, label the barchart with Title = ” Enrollment of BS 
Computer Science" horizontal axis = “Curriculum Year” and vertical axis = 
“number of students”
...
```
\hfill\break
b. Plot the data using a pie chart. Add labels, colors and legend. Write the 
...
```
\hfill\break
b. Colored histogram with different number of bins. hist(mtcars$mpg, breaks=12, \
col="red") 
...
```
\hfill\break
c. Add a Normal Curve x <- mtcars$mpg h<-hist(x, breaks=10, col="red", 
xlab="Miles Per Gallon", main="Histogram with Normal Curve") xfit<-seq(min(x),
max(x),length=40) yfit<-dnorm(xfit,mean=mean(x),sd=sd(x)) 
...
Write the codes and its result. Example: setosa <- colMeans(setosa[sapply
(setosaDF,is.numeric)])
